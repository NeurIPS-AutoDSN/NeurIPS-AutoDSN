...
parser.add_argument('--batch_size', type=int, default=2048*5)
...
def main():
    ...
    data_path_dict = {
        'criteo': './data/train.csv',
        'movielens1M': './data/MovieLens1M.csv',
        'movielens100K': './data/MovieLens100K.csv',
    }
    args.device = 'cuda:0'
    args.dataset_path = data_path_dict[args.dataset_name]
    ...
    # load genotype saved
    # genotype = np.load(os.path.join(args.genotype_path, 'genotype.npy'))

    genotype = np.load(os.path.join('./save/criteo_weight.npy')).reshape(-1, 1)
    mask_num = 0
    sorted_index = np.argsort(genotype[:, 0])
    for k in range(mask_num):
        genotype[sorted_index[k]] = 0.0000
    logging.info('genotype = %s', genotype[:, 0])
    model = Network(field_dims, args.embed_dim, genotype).to(device)
